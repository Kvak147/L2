...
class RedBlackTree:
    def __init__(self):
        self.NIL_LEAF = TreeNode(None)  # Листовой узел NIL
        self.NIL_LEAF.color = 'black'  # Листовой узел всегда черный
    ...
    def insert(self, key):
        ...
        if parent is None:  # Дерево пустое
            self.root = new_node
    ...
    def fix_insert(self, node):
        while node != self.root and node.parent.color == 'red':
            if node.parent == node.parent.parent.left:  # Если родитель - левый сын
                ...
                else:  # Случай 2 и 3: дядя черный
                    if node == node.parent.right:  # Случай 2: узел - правый сын
                        node = node.parent
                        self.left_rotate(node)  # Случай 3: узел - левый сын
                    node.parent.color = 'black'
                    node.parent.parent.color = 'red'
                    self.right_rotate(node.parent.parent)
            else:  # Если родитель - правый сын (симметрично)
    ...
    def left_rotate(self, x):
        ...
        y.parent = x.parent
        if x.parent is None:  # Если x - корень
            self.root = y
    ...
    def right_rotate(self, y):
        ...
        x.parent = y.parent
        if y.parent is None:  # Если y - корень
            self.root = x
